...
EVENT_MAX = [e*action_frequency for e in EVENT_MAX]
...
class CustomEnv(gym.Env):
    def __init__(self):
        ...
        for proc in psutil.process_iter():
            if 'xapian_integrated' in proc.name():
                self.pid = proc.pid
                print(self.pid)
        if self.pid == 0 :
    ...
    def getReward(self, ignoreAction = 0):
        ...
        sjrn99 = np.percentile(containerReward['reward'], 90)
        ...
        if qos > qosTarget:
            ...
            reward = qosTarget/qos + (20/self.appCacheWays)/4 + (24/len(self.cores))*2
        if ignoreAction != 0:
            reward = -10
