...
class learn_continuous_tasks:
    def __init__(self,
        ...
        self.action_space_way = way_space
        ...
        self.save_model_wayuency = 50000 # steps
        ...
        self.low = np.array( [ np.min(self.action_space_core), np.min(self.action_space_way) ] )
        self.high = np.array( [ np.max(self.action_space_core), np.max(self.action_space_way) ] )
        self.actions_range = np.subtract(self.high, self.low)

        self.num_actions_pad = [ \
                [len(self.action_space_core), len(self.action_space_way)]\
                ] * self.num_apps
